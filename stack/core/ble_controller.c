...
{
    radio_cfg_drate_plen_and_enable_mode(mode,
                                         controller_radio_data_rate_for_phy(phy),
                                         controller_radio_preamble_for_phy(phy));
}
...
static ble_ll_data_header_t controller_conn_header_for_state(uint8_t llid,
...
void controller_disconnect_internal(void)
{
    ...
    radio_disable();
    radio_set_shorts(0U);
    radio_enable_interrupt_mask(RADIO_INTENSET_CRCOK_Msk |
...
static void controller_prestage_conn_response_from_header(void)
{
    ...
    if (m_ctrl_rt.has_pending_conn_tx_pdu)
    {
        ...
        m_ctrl_rt.conn_tx_pdu.header =
            controller_conn_header_for_state(m_ctrl_rt.conn_tx_pdu.header.llid,
                                             next_expected_rx_sn,
                                             tx_sn);
    ...
    {
        m_ctrl_rt.conn_tx_pdu.header =
            controller_conn_header_for_state(BLE_LLID_CONTINUATION,
                                             next_expected_rx_sn,
                                             tx_sn);
...
static void controller_start_connection_event(void)
{
    ...
    radio_enable_interrupt_mask(RADIO_INTENSET_BCMATCH_Msk |
...
void TIMER2_IRQHandler(void)
{
    ...
    if (m_link.pending_phy_update.valid &&
        (m_link.event_counter == m_link.pending_phy_update.instant))
    {
