...
uint16_t ble_uuid_encoded_len(const ble_uuid_t *p_uuid)
{
    ...
    if ((p_uuid->type == BLE_UUID_TYPE_VENDOR_16) && m_host.common.local_vendor_uuid_base_set)
    {
        return BLE_UUID128_LEN;
    }
...
bool ble_uuid_encode(const ble_uuid_t *p_uuid, uint8_t *p_dst)
{
    ...
    if ((p_uuid->type == BLE_UUID_TYPE_VENDOR_16) && m_host.common.local_vendor_uuid_base_set)
    {
        (void)memcpy(p_dst, m_host.common.local_vendor_uuid_base, BLE_UUID128_LEN);
...
void ble_uuid_set_vendor_base(const uint8_t uuid128[BLE_UUID128_LEN])
{
    if (uuid128 == NULL)
    {
        m_host.common.local_vendor_uuid_base_set = false;
        (void)memset(m_host.common.local_vendor_uuid_base, 0, sizeof(m_host.common.local_vendor_uuid_base));
        return;
    }

    (void)memcpy(m_host.common.local_vendor_uuid_base, uuid128, BLE_UUID128_LEN);
    m_host.common.local_vendor_uuid_base_set = true;
}
