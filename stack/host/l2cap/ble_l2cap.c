...
static uint16_t ble_l2cap_process_signaling_pdu(const uint8_t *p_sig,
                                                uint16_t sig_len,
                                                uint8_t *p_rsp,
...
void ble_l2cap_process_conn_data_pdu(const uint8_t *p_payload, uint8_t pdu_len)
{
    ...
    p_l2cap_payload = &p_payload[BLE_L2CAP_HDR_LEN];

    if (l2cap_cid == BLE_L2CAP_CID_ATT)
    {
        rsp_cid = BLE_L2CAP_CID_ATT;
        rsp_len = (m_link.role == BLE_GAP_ROLE_CENTRAL)
                      ? ble_gatt_client_process_att_pdu(p_l2cap_payload, l2cap_len, rsp, sizeof(rsp))
                      : ble_gatt_server_process_att_pdu(p_l2cap_payload, l2cap_len, rsp, sizeof(rsp));
    }
    else if (l2cap_cid == BLE_L2CAP_CID_SIGNALING)
    {
        rsp_cid = BLE_L2CAP_CID_SIGNALING;
        rsp_len = ble_l2cap_process_signaling_pdu(p_l2cap_payload, l2cap_len, rsp, sizeof(rsp));
    }
